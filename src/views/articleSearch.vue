<template>
  ...
  <div v-if="NothingFoundVue"><NothingFoundVue/></div> 
      <b-modal id="modal-multi-Art" title="Login into Your DGA Account" hide-footer>
  
        <form @submit="onLoginArt">
          <div style="margin: 10px 0 10px 200px;" v-if="loeding" class="loader"></div>
  ...
  export default {
    ...
    async created() {
      ...
      this.$bus.$on('logged', () => {
        ...
        await fetch("  http://46.105.36.240:3000/search/cathegoies/"+ this.catId+"/articles/"+ this.ArtName)
          .then(response => response.json())
          .then((data) => {
            if (data == "") {
              this.loading = false;
              this.NothingFoundVue = true
            } else {
              this.loading = false;
              ...
              for (let p = 0; p < this.allArticles.length; p++) {
                ...
                ) 
                {
                  ...
                  console.error( this.allArticles);
                }
              }
