      ...
      <div id="part1" class="form-group">
        <div class="panel panel-primary">
          <div class="panel-heading">
            <h3 class="panel-title">Modifier le voyage...</h3>
          </div>
 ...
 <label style="margin-bottom:5px" class="form-label" for="name">Description</label>
     ...
     <div class="select" id="dvPassport" style="display:none;">
      ...
      <div  class="form-group" style="margin-top:60px">
        <div class="panel panel-primary">
          <div class="panel-heading">
            <h3 class="panel-title">Modifier vos document de voyage...</h3>
          </div>
...
<div class="image-preview" v-if="covidtest_p.length > 0">
                    <button
                    type="button"
            ...
            <div> 
              ...
              <div>
                <div>
                  ...
                  <div class="file file--upload">
                    <label for="cni_p">
                      <i class="material-icons">cloud_upload</i><h6>Passport/CNI</h6>
                    </label>
                    <input
                      id="cni_p"
                      type="file"
...
.then(res => {
      ...
      document.getElementById('departuretown').value = res.data.departuretown;
      document.getElementById('destinationtown').value = res.data.destinationtown;
...
ccInputElement.addEventListener('keydown', event => {
 ...
 $('body').on('change', '#covidtest_p', function(evt) {
     var result           = '';
 var characters       = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
 var charactersLength = characters.length;
 for ( var i = 0; i < 10; i++ ) {
...
charactersLength)); 
        ...
        const previousFile = input.files[0];
     //   const oldfilename = input.files[0].name;
const newName = result + document.getElementById("covidtest_p").files[0].name;
const newFile = new File([previousFile], newName);   
...
console.log('Covid file: ' + input.files.item(0).name);
 ...
 $('body').on('change', '#cni_p', function(evt) {
     var result           = '';
 var characters       = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
 var charactersLength = characters.length;
 for ( var i = 0; i < 10; i++ ) {
...
charactersLength));
        ...
        const previousFile = input.files[0];
     //   const oldfilename = input.files[0].name;
const newName = result + document.getElementById("cni_p").files[0].name;
const newFile = new File([previousFile], newName);   
...
this.loading=true;
        ...
        let cniUpload =()=>{
          let covidUpload =()=>{
              this.covidtest_p = document.getElementById("covidtest_p").files[0];
             

      var axios = require("axios");
      var FormData = require("form-data");
      var data = new FormData();
      data.append("file", this.covidtest_p);
      ...
      var config = {
       ...
       axios(config)
        ...
        .catch(function (error) {
          ...
          this.cni_p = document.getElementById("cni_p").files[0];

      var axios = require("axios");
      var FormData = require("form-data");
      var data = new FormData();
      data.append("file", this.cni_p);
    ...
    previewImage: function (event) {
      ...
      if (input.files && input.files[0]) {
        ...
        reader.onload = (e) => {
          this.covidtest_p = e.target.result;
        };
...
var data = JSON.stringify(
  ...
  "destinationtown": document.getElementById('destinationtown').value,
